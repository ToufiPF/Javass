...
/**
 * Launcher La classe qui permet de lancer le menu principal du jeu. S'occupe de
 * créer une partie ou de la rejoindre en réseau.
 *
...
public final class Launcher extends Application {
    ...
    private final static BooleanProperty tryAgainProperty = new SimpleBooleanProperty(
            false);
    ...
    private static boolean isSeedValid(String txt) {
        if (txt.length() == 0) {
            ...
            if (!(Character.isDigit(txt.charAt(0)) || txt.charAt(0) == '+'
                    || txt.charAt(0) == '-'))
                return false;
    ...
    private VBox createCreateGameMenu() {
        ...
        for (int i = 0; i < PlayerId.COUNT; ++i) {
            ...
            typeChoices.add(new ChoiceBox<>(FXCollections.observableArrayList(
                    PlayerSpecificator.HUMAN.frenchName(),
                    PlayerSpecificator.SIMULATED.frenchName(),
                    PlayerSpecificator.REMOTE.frenchName())));
            ...
            IADifficultySpinners.get(index).visibleProperty()
                    .bind(Bindings.equal(
                            PlayerSpecificator.SIMULATED.frenchName(),
                            typeChoices.get(index).valueProperty()));
            ...
            ipFields[index].visibleProperty()
                    .bind(Bindings.equal(PlayerSpecificator.REMOTE.frenchName(),
                            typeChoices.get(index).valueProperty()));
        ...
        Label seedLbl = new Label(
                "Entrez la graîne de la partie : (laisser vide pour aléatoire) : ");
        ...
        launchGameBtn.setOnMouseClicked(e -> {
            ...
            for (int i = 0; i < PlayerId.COUNT; ++i) {
                ...
                if (typeChoices.get(i).getValue()
                        .equals(PlayerSpecificator.HUMAN.frenchName())) {
                    args.add("h:" + name);
                } else if (typeChoices.get(i).getValue()
                        .equals(PlayerSpecificator.SIMULATED.frenchName())) {
                    args.add("s:" + name + ":"
                            + IADifficultySpinners.get(i).getValue()
                                    * ITERATIONS_BY_IA_LEVEL);
                } else {
    ...
    private VBox createJoinGameMenu() {
        ...
        Label lbl = new Label(
                "La partie commencera quand un client se connectera à votre serveur.");
        menu.getChildren().add(lbl);
        return menu;
    }
